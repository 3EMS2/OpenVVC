...
static inline struct MV
mv_clip_periodic(struct MV src)
{
    struct MV dst;
...
//TODOamvr
static inline struct MV
round_affine_mv(const struct MV mv, uint8_t prec_amvr)
{
    struct MV tmp = drv_round_to_precision_mv(mv, MV_PRECISION_INTERNAL, prec_amvr);

    return tmp;
}

static inline struct MV
round_affine_mv2(struct MV mv)
{
                    ...
                    uint8_t cand_msk, struct MV *const dst)
{
    ...
    if (cand_c0 | cand_c01) {
        ...
        if (cand_c0 && cand_c01 && !tmvp->col_ref_l0) {
            ...
            dst[0] = tmvp_rescale(mv.mv, scale);
            ...
            dst[1] = tmvp_rescale(mv.mv, scale);
            if (!((dist_ref == 0) ^ (mv.z == 0)))
                goto found;
...
derive_affine_delta_mvs(const struct CPMV *const cinfo,
                        uint8_t log2_pb_w, uint8_t log2_pb_h,
                        uint8_t affine_type)
{
    struct AffineDeltaMV dmv;
    struct MV delta_mv_h, delta_mv_v;
    delta_mv_h = cinfo->lt;
    delta_mv_v = cinfo->lt;
                    ...
                    enum CandName cand_name, uint8_t log2_ctu_s)
{
    ...
    uint8_t ref_idx = ngh_cp->ref_idx;
    ...
    struct AffineDeltaMV delta_mv = derive_affine_delta_mvs(&ngh_cp->cp_mv, log2_ngh_w, log2_ngh_h,
                                                            ngh_affine_type);

    struct MV tmp = {0}, lt_mv = {0};
    ...
    lt_mv.x = (uint32_t)ngh_cp->cp_mv.lt.x << AFFINE_SHIFT;
    lt_mv.y = (uint32_t)ngh_cp->cp_mv.lt.y << AFFINE_SHIFT;
    ...
    dst_cp.cp_mv.lt = round_affine_mv2(lt_mv);
    dst_cp.cp_mv.lt = clip_mv(dst_cp.cp_mv.lt);
    dst_cp.ref_idx = ref_idx;
    dst_cp.mv_spec = ngh_cp->mv_spec;

    tmp.x = lt_mv.x + (int32_t)((uint32_t)delta_mv.h.x << log2_pb_w);
    tmp.y = lt_mv.y + (int32_t)((uint32_t)delta_mv.h.y << log2_pb_w);

    dst_cp.cp_mv.rt = round_affine_mv2(tmp);
    dst_cp.cp_mv.rt = clip_mv(dst_cp.cp_mv.rt);

    if (affine_type == AFFINE_3CP) {
        tmp.x = lt_mv.x + (int32_t)((uint32_t)delta_mv.v.x << log2_pb_h);
        tmp.y = lt_mv.y + (int32_t)((uint32_t)delta_mv.v.y << log2_pb_h);

        dst_cp.cp_mv.lb = round_affine_mv2(tmp);
        dst_cp.cp_mv.lb = clip_mv(dst_cp.cp_mv.lb);
    }
                       ...
                       uint8_t affine_type, uint8_t log2_ctu_s)
{
    ...
    if (avail_affine) {
        ...
        if (avail_rpl) {
            ...
            if (ngh_cp_info.ref_idx == ref_idx) {
                *dst_cpmv = derive_cp_from_cand(&ngh_cp_info, pb, ngh_pb, affine_type,
                                                affine_info->type, cand_name, log2_ctu_s).cp_mv;
                return 1;
            }
        }
        ...
        if (avail_rpl_opp) {
            ...
            if (ngh_cp_info.ref_idx == ref_opp_idx) {
                *dst_cpmv = derive_cp_from_cand(&ngh_cp_info, pb, ngh_pb, affine_type,
                                                affine_info->type, cand_name, log2_ctu_s).cp_mv;
                return 1;
            }
               ...
               uint8_t inter_dir, uint8_t affine_type)
{
    ...
    /* Affine above cand */
    cand_aff_abv = derive_affine_mvp_cand(affine_ctx, &cp_info, pb_info, B0,
                                          inter_dir, ref_idx, ref_opp_idx,
                                          rpl0_cand, rpl1_cand, aff_cand_list,
                                          affine_type, log2_ctb_s);
    if (!cand_aff_abv) {
        cand_aff_abv = derive_affine_mvp_cand(affine_ctx, &cp_info, pb_info, B1,
                                              inter_dir, ref_idx, ref_opp_idx,
                                              rpl0_cand, rpl1_cand, aff_cand_list,
                                              affine_type, log2_ctb_s);
        if (!cand_aff_abv) {
            cand_aff_abv = derive_affine_mvp_cand(affine_ctx, &cp_info, pb_info, B2,
                                                  inter_dir, ref_idx, ref_opp_idx,
                                                  rpl0_cand, rpl1_cand, aff_cand_list,
                                                  affine_type, log2_ctb_s);
        }
    ...
    if (cand_msk == 0x7 || ((cand_msk == 0x3) && (affine_type == AFFINE_2CP))) {
        if (nb_cand++ == mvp_idx) {
            cp_info = cpmv_cand;
            return cp_info;
        }
...
static inline struct MV
mv_internal_to_integer(struct MV mv)
{
    mv.x += 7 + (mv.x < 0);
    mv.y += 7 + (mv.y < 0);
    mv.x >>= 4;
...
derive_sbtmvp_cand_pos(uint8_t x0, uint8_t y0, uint8_t log2_pu_w, uint8_t log2_pu_h,
                       struct MV mv_offset, int16_t ctu_w, int16_t ctu_h, uint8_t is_bnd)
{
...
found:
    return mv_internal_to_integer(a1_mv.mv);
}
                ...
                uint8_t cand_msk, struct MV *const dst)
{
    ...
    int32_t dist_ref_opp = inter_ctx->inter_params.dist_ref_1[0];
#if 0
    int32_t dist_ref = rpl_idx == RPL_0 ? inter_ctx->inter_params.dist_ref_0[0]
                                        : inter_ctx->inter_params.dist_ref_1[0];

    int32_t dist_ref_opp = rpl_idx == RPL_0 ? inter_ctx->inter_params.dist_ref_1[0]
                                            : inter_ctx->inter_params.dist_ref_0[0];
#endif
    uint8_t cand_c0  = cand_msk & 0x1;
    uint8_t cand_c01 = cand_msk & 0x2;
                     ...
                     uint8_t cand_msk, struct MV *const dst)
{
    ...
    if (rpl_idx == RPL_0) {
        dist_ref  = inter_ctx->inter_params.dist_ref_0[0];
        mvs       = tmvp->ctb_mv0;

        dir  = cand_msk & 0x1;
    } else {
        dist_ref  = inter_ctx->inter_params.dist_ref_1[0];
        mvs       = tmvp->ctb_mv1;
    ...
    if (dir) {
        ...
        struct TMVPMV mv = mvs[c0_pos];
        int32_t dist_col = mv.z;

        if (((dist_ref == 0) ^ (dist_col == 0)))
            goto not_found_lt;

        int16_t scale = derive_tmvp_scale(dist_ref, dist_col);

        *dst = tmvp_rescale(mv.mv, scale);
                         ...
                         uint8_t cand_rpl0, uint8_t cand_rpl1)
{
    ...
    struct MV mv_offset = derive_sbtmvp_mv_offset(inter_ctx, &pb_info, cand_rpl0, cand_rpl1);
    ...
    if (cand_msk) {
        if (inter_ctx->inter_params.low_delay) {
            ...
            inter_dir  = sbtmvp_from_ldc(inter_ctx, tmvp, center_pos,
                                         RPL_0, cand_msk, col_mv);

            mv_info->mv0.mv = col_mv[0];
            mv_info->mv1.mv = col_mv[1];
        } else {
            struct MV col_mv[2];
            inter_dir  = sbtmvp_from_same_rpl(inter_ctx, tmvp, center_pos,
                                              RPL_0, cand_msk, col_mv);
            inter_dir |= sbtmvp_from_same_rpl(inter_ctx, tmvp, center_pos,
                                              RPL_1, cand_msk, &col_mv[1]);
            mv_info->mv0.mv = col_mv[0];
            mv_info->mv1.mv = col_mv[1];
        }
    }
               ...
               uint8_t log2_pu_w, uint8_t log2_pu_h)
{
    ...
    OVMV mv0 = {0};
    struct TMVPMV tmvpmv0 = {{0}, .z = inter_ctx->inter_params.dist_ref_0[0]};
...
derive_sub_block_mvs_p(struct InterDRVCtx *inter_ctx,
                       ...
                       uint8_t x0, uint8_t y0,
                       uint8_t log2_pu_w, uint8_t log2_pu_h,
                       struct MV mv_offset, const struct MergeInfo *const main_mv)
{
    int nb_sb_w = OVMAX((1 << log2_pu_w) >> LOG2_SBTMVP_S, 1);
    int nb_sb_h = OVMAX((1 << log2_pu_h) >> LOG2_SBTMVP_S, 1);
    ...
    for (y = start_y, i = 0; i < nb_sb_h; ++i, y += sb_h) {
        for (x = start_x, j = 0; j < nb_sb_w; ++j, x += sb_w) {
            ...
            if (cand_msk) {
                if (inter_ctx->inter_params.low_delay) {
                    struct MV col_mv[2];
                    inter_dir  = sbtmvp_from_ldc(inter_ctx, tmvp, col_pos,
                                                 RPL_0, cand_msk, col_mv);

                    mv0.mv = col_mv[0];
                } else {
                    struct MV col_mv[2];
                    inter_dir  = sbtmvp_from_same_rpl(inter_ctx, tmvp, col_pos,
                                                      RPL_0, cand_msk, col_mv);
                    inter_dir |= sbtmvp_from_same_rpl(inter_ctx, tmvp, col_pos,
                                                      RPL_1, cand_msk, &col_mv[1]);
                    mv0.mv = inter_dir & 0x1 ? col_mv[0] : col_mv[1];
                }
            }

            if (!(inter_dir & 0x1)) {
                mv0.mv = main_mv->mv0.mv;
                inter_dir = 0x1;
            }
               ...
               uint8_t x0, uint8_t y0, uint8_t log2_pu_w, uint8_t log2_pu_h)
{
    ...
    OVMV mv1 = {0};
    struct TMVPMV tmvpmv0 = {{0}, .z = inter_ctx->inter_params.dist_ref_0[0]};
    struct TMVPMV tmvpmv1 = {{0}, .z = inter_ctx->inter_params.dist_ref_1[0]};
    ...
    switch (model_idx)
    {
        case 0:
            ...
            if (mi.mv0[CP_LT].ref_idx >= 0 && mi.mv0[CP_LT].ref_idx == mi.mv0[CP_RT].ref_idx) {
                ...
                mv0[CP_LT] = mi.mv0[CP_LT].mv;
                mv0[CP_RT] = mi.mv0[CP_RT].mv;
            ...
            if (mi.mv1[CP_LT].ref_idx >= 0 && mi.mv1[CP_LT].ref_idx == mi.mv1[CP_RT].ref_idx) {
                ...
                mv1[CP_LT] = mi.mv1[CP_LT].mv;
                mv1[CP_RT] = mi.mv1[CP_RT].mv;
                              ...
                              struct AffineMergeInfo *const affmrg_info)
{
    ...
    uint8_t ref_idx0 = 0;
    uint8_t ref_idx1 = 0;
    struct MVProperty mv_spec0 ={0};
    struct MVProperty mv_spec1 ={0};
    ...
    switch (model_idx) {
        case 0:
            ...
            if (mi.mv0[CP_LT].ref_idx >= 0 && mi.mv0[CP_LT].ref_idx == mi.mv0[CP_RT].ref_idx
                ...
                ref_idx0 = mi.mv0[0].ref_idx;
                mv_spec0 = mi.mv0[CP_LT].mv_spec;
            ...
            if (mi.mv1[CP_LT].ref_idx >= 0 && mi.mv1[CP_LT].ref_idx == mi.mv1[CP_RT].ref_idx
                ...
                ref_idx1 = mi.mv1[0].ref_idx;
                mv_spec1 = mi.mv1[CP_LT].mv_spec;
        ...
        case 3:
            if (mi.mv0[CP_RT].ref_idx >= 0 && mi.mv0[CP_RT].ref_idx == mi.mv0[CP_LB].ref_idx
                ...
                ref_idx0 = mi.mv0[CP_RT].ref_idx;
                mv_spec0 = mi.mv0[CP_RT].mv_spec;
            ...
            if (mi.mv1[CP_RT].ref_idx >= 0 && mi.mv1[CP_RT].ref_idx == mi.mv1[CP_LB].ref_idx
                ...
                ref_idx1 = mi.mv1[CP_RT].ref_idx;
                mv_spec1 = mi.mv1[CP_RT].mv_spec;
                       ...
                       const int mrg_idx)
{
    ...
    /* derive affine control points from neighbours */
    {
        ...
        if (inter_ctx->inter_params.tmvp_enabled) {
            const struct VVCTMVP *const tmvp = &inter_ctx->tmvp_ctx;
            const struct TMVPPos pos = compute_tmpv_coord(pb_info);
                     ...
                     uint8_t mv_broad)
{
    ...
    uint8_t ref_idx = cinfo->ref_idx;

    if (!mv_broad) {
        int i, j;
        struct MV accu_mv_v;

        accu_mv_v.x = ((uint32_t)cinfo->cp_mv.lt.x << AFFINE_SHIFT) + delta_mv.h.x * HALF_SB_SIZE
                                                    + delta_mv.v.x * HALF_SB_SIZE;
        accu_mv_v.y = ((uint32_t)cinfo->cp_mv.lt.y << AFFINE_SHIFT) + delta_mv.h.y * HALF_SB_SIZE
        ...
        for (i = 0; i < nb_sb_h; ++i) {
            ...
            for (j = 0; j < nb_sb_w; ++j) {
                ...
                mv_dst = round_affine_mv2(accu_mv_h);
                mv_dst = clip_mv(mv_dst);

                mv_buff[j].mv = mv_dst;
                mv_buff[j].ref_idx = ref_idx;
                mv_buff[j].mv_spec = cinfo->mv_spec;
        ...
        tmp.x = (uint32_t)cinfo->cp_mv.lt.x << AFFINE_SHIFT;
        tmp.y = (uint32_t)cinfo->cp_mv.lt.y << AFFINE_SHIFT;

        tmp.x += (int32_t)((uint32_t)delta_mv.h.x << log2_cu_w) >> 1;
        tmp.y += (int32_t)((uint32_t)delta_mv.h.y << log2_cu_w) >> 1;

        tmp.x += (int32_t)((uint32_t)delta_mv.v.x << log2_cu_h) >> 1;
        tmp.y += (int32_t)((uint32_t)delta_mv.v.y << log2_cu_h) >> 1;

        tmp = round_affine_mv2(tmp);

        center_mv.mv = clip_mv(tmp);

        center_mv.ref_idx = ref_idx;
        center_mv.mv_spec = cinfo->mv_spec;
                ...
                uint8_t inter_dir)
{
    ...
    if (inter_dir == 0x3) {
        ...
        const struct AffineDeltaMV dmv_1 = derive_affine_delta_mvs(&cinfo[1].cp_mv,
                                                                   log2_cu_w, log2_cu_h,
                                                                   affine_type);
...
{
    int x, y, i;
    struct MV quad_dmv_h;
    struct MV quad_dmv_v;
    ...
    for (i = 0; i < nb_sb_h; ++i) {
        for (j = 0; j < nb_sb_w; ++j) {
            ...
            if (!(((x0+4*j)>>2)& 0x1) && !((((y0+4*i)>>2)& 0x1))) {
                struct TMVPMV tmvpmv0 = {.mv=mv0.mv, .z=inter_ctx->inter_params.dist_ref_0[ref_idx0]};
                struct TMVPMV tmvpmv1 = {.mv=mv1.mv, .z=inter_ctx->inter_params.dist_ref_1[ref_idx1]};
                   ...
                   uint8_t inter_dir)
{
    ...
    for (i = 0; i < nb_sb_h; i += 2) {
        for (j = 0; j < nb_sb_w; j += 2) {
            struct OVMV mv0 = mv_buff0[j];
            struct OVMV mv1 = mv_buff1[j];

            mv0.mv.x += mv_buff0[j + 35].mv.x;
            mv0.mv.y += mv_buff0[j + 35].mv.y;
            ...
            mv1.mv.x += mv_buff1[j + 35].mv.x;
            mv1.mv.y += mv_buff1[j + 35].mv.y;
...
static inline uint8_t
mv_cmp(const struct MV a, const struct MV b)
{
     uint8_t is_eq;
     is_eq = a.x == b.x;
     is_eq &= a.y == b.y;
...
check_affine_prof(const struct AffineMergeInfo *const affine_info, uint8_t rpl_idx)
{
    ...
    if (affine_info->affine_type == AFFINE_3CP) {
        prof_enabled &= !(mv_cmp(cpinfo->cp_mv.lt, cpinfo->cp_mv.rt) && mv_cmp(cpinfo->cp_mv.lt, cpinfo->cp_mv.lb));
    } else {
        prof_enabled &= !mv_cmp(cpinfo->cp_mv.lt, cpinfo->cp_mv.rt);
    }
                 ...
                 uint8_t inter_dir)
{
    ...
    struct MVProperty mv_spec = {
        .prec_amvr = mvp_info->prec_amvr,
        .bcw_idx_plus1 = 3
    };
    ...
    if (inter_dir & 0x1) {
        ...
        const struct CPMV *const mvd = &mvp_info->data.cp_mvd_info.mvd;

        uint8_t opp_ref_idx0 = inter_ctx->inter_params.rpl0_opp[ref_idx0];
        cp_info->cp_mv = drv_affine_mvp(inter_ctx, affine_ctx, x_pb, y_pb,
                                        nb_pb_w, nb_pb_h, log2_cu_w, log2_cu_h,
                                        ref_idx0, opp_ref_idx0, mvp_idx0,
                                        inter_dir & 0x1, affine_type);

        struct MV mvd_lt = drv_change_precision_mv(mvd->lt, prec_amvr, MV_PRECISION_INTERNAL);
        struct MV mvd_rt = drv_change_precision_mv(mvd->rt, prec_amvr, MV_PRECISION_INTERNAL);

        cp_info->cp_mv.lt = round_affine_mv(cp_info->cp_mv.lt, prec_amvr);
        cp_info->cp_mv.rt = round_affine_mv(cp_info->cp_mv.rt, prec_amvr);

        cp_info->cp_mv.lt.x += mvd_lt.x;
        cp_info->cp_mv.lt.y += mvd_lt.y;

        cp_info->cp_mv.rt.x += mvd_lt.x;
        cp_info->cp_mv.rt.y += mvd_lt.y;
        cp_info->cp_mv.rt.x += mvd_rt.x;
        cp_info->cp_mv.rt.y += mvd_rt.y;

        cp_info->cp_mv.lt = mv_clip_periodic(cp_info->cp_mv.lt);
        cp_info->cp_mv.rt = mv_clip_periodic(cp_info->cp_mv.rt);

        cp_info->ref_idx = ref_idx0;
        cp_info->mv_spec = mv_spec;

        if (affine_type == AFFINE_3CP) {
            struct MV mvd_lb = drv_change_precision_mv(mvd->lb, prec_amvr, MV_PRECISION_INTERNAL);
            cp_info->cp_mv.lb = round_affine_mv(cp_info->cp_mv.lb, prec_amvr);

            cp_info->cp_mv.lb.x +=  mvd_lt.x;
            cp_info->cp_mv.lb.y +=  mvd_lt.y;
            cp_info->cp_mv.lb.x +=  mvd_lb.x;
            cp_info->cp_mv.lb.y +=  mvd_lb.y;
            cp_info->cp_mv.lb = mv_clip_periodic(cp_info->cp_mv.lb);
        }
    }
    ...
    if (!prof_dir) {
        ...
        if (prof_dir & 0x1) {
            dmv_0 = derive_affine_delta_mvs(&cinfo[0].cp_mv, log2_cu_w, log2_cu_h, affine_type);
        }

        /*FIXME might be unused */
        if (prof_dir & 0x2) {
            dmv_1 = derive_affine_delta_mvs(&cinfo[1].cp_mv, log2_cu_w, log2_cu_h, affine_type);
        }
                 ...
                 uint8_t inter_dir)
{
    ...
    struct MVProperty mv_spec = {
        .prec_amvr = mvp_info->prec_amvr,
        .bcw_idx_plus1 = mvp_info->bcw_idx + 1,
    };
    ...
    if (inter_dir & 0x1) {
        ...
        cp_info->cp_mv.rt.x += lt.x;
        cp_info->cp_mv.rt.y += lt.y;
        cp_info->cp_mv.rt.x += rt.x;
        cp_info->cp_mv.rt.y += rt.y;
                       ...
                       uint8_t merge_idx)
{
    ...
    uint8_t nb_pb_w = (1 << log2_cu_w) >> 2;
    uint8_t nb_pb_h = (1 << log2_cu_h) >> 2;
    uint8_t sbtmvp_enabled = inter_ctx->inter_params.sbtmvp_enabled;
    uint8_t is_sbtmvp = 0;
    uint8_t log2_pmerge_lvl = inter_ctx->inter_params.log2_parallel_merge_level;
    ...
    if (sbtmvp_enabled) {
        struct OVMVCtx *const mv_ctx0 = &inter_ctx->mv_ctx0;
        struct MergeInfo mv_info = {0};
        struct MV mv_offset = {0};
