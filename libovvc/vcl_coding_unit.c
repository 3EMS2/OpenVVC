            ...
            uint8_t log2_cb_w, uint8_t log2_cb_h)
{
    ...
    if (ctu_dec->coding_unit != &coding_unit_intra_c) {
        ...
        if (qp_grp_start) {
            ...
            int pred_qp = ((y0 ? ctu_dec->drv_ctx.qp_map_x[qp_grp_x0 >> log2_min_cb_s] : ctu_dec->qp_ctx2.current_qp) +
                           (x0 ? ctu_dec->drv_ctx.qp_map_y[qp_grp_y0 >> log2_min_cb_s] : ctu_dec->qp_ctx2.current_qp) + 1) >> 1;
            ctu_dec->read_qp = 1;
            ctu_dec->qp_ctx2.current_qp = pred_qp;
        }
        ...
        ctu_dec->dequant_cb = ov_clip(ctu_dec->qp_ctx.chroma_qp_map_cb[base_qp_c] + ctu_dec->qp_ctx2.dqp_cb + ctu_dec->qp_ctx.cb_offset, -qp_bd_offset, 63) + qp_bd_offset;
        ctu_dec->dequant_cr = ov_clip(ctu_dec->qp_ctx.chroma_qp_map_cr[base_qp_c] + ctu_dec->qp_ctx2.dqp_cr + ctu_dec->qp_ctx.cr_offset, -qp_bd_offset, 63) + qp_bd_offset;
        ctu_dec->dequant_joint_cb_cr = ov_clip(ctu_dec->qp_ctx.chroma_qp_map_jcbcr[base_qp_c] + ctu_dec->qp_ctx2.dqp_jcbcr + ctu_dec->qp_ctx.jcbcr_offset, -qp_bd_offset, 63) + qp_bd_offset;
...
check_nz_mvd_p(const struct MV *const mvd)
{
                   ...
                   struct MVPInfoP mvp_info, uint8_t inter_dir)
{
    ...
    if (mvp_info.cu_type == OV_AFFINE) {
        ...
        if (inter_dir & 0x1) {
            ...
            mv_info.mv0.mv_spec.prec_amvr = mvp_info.prec_amvr;
        } else {
            ...
            mv_info.mv1.mv_spec.prec_amvr = mvp_info.prec_amvr;
        }
    }
...
static inline uint8_t
check_nz_mvd_b(const struct MV *const mvd0, const struct MV *const mvd1,
...
static inline uint8_t
check_nz_mvd_smvd(const struct MV *const mvd0)
{
    int32_t mvd_not_zero = 0;
    mvd_not_zero |= (mvd0->x | mvd0->y);
    return (uint8_t)!!mvd_not_zero;
                       ...
                       uint8_t nb_active_ref0_min1, uint8_t nb_active_ref1_min1)
{
    ...
    if (affine_flag) {
        ...
        if (inter_ctx->inter_params.bdof_enabled) {
            ...
            uint8_t ciip_flag = 0;
            uint8_t bcw_flag = (mv_info.mv0.mv_spec.bcw_idx_plus1 != 0 && mv_info.mv0.mv_spec.bcw_idx_plus1 != 3);
                        ...
                        uint8_t skip_flag, uint8_t merge_flag)
{
    ...
    if (merge_flag) {
        ...
        if (mrg_data.merge_type == SB_MERGE) {
            ...
            mv_info.mv0.mv_spec.bcw_idx_plus1 = 0;
            mv_info.mv1.mv_spec.bcw_idx_plus1 = 0;
