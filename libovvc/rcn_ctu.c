...
rcn_write_ctu_to_frame(const struct OVRCNCtx *const rcn_ctx, uint8_t log2_ctb_s)
{
    ...
    const OVSample *src_y  = &ctu_buff->y [0];
    const OVSample *src_cb = &ctu_buff->cb[0];
    const OVSample *src_cr = &ctu_buff->cr[0];
...
rcn_intra_line_to_ctu(const struct OVRCNCtx *const rcn_ctx, int x_l, uint8_t log2_ctb_s)
{
    ...
    OVSample *dst_y  =  ctu_buff->y  - ctu_buff->stride;
    OVSample *dst_cb =  ctu_buff->cb - ctu_buff->stride_c;
    OVSample *dst_cr =  ctu_buff->cr - ctu_buff->stride_c;
...
rcn_ctu_to_intra_line(const struct OVRCNCtx *const rcn_ctx, int x_l, uint8_t log2_ctu_s)
{
    ...
    _src = &ctu_buff->y[(max_cu_width_l - 1) * ctu_buff->stride];
    memcpy(&intra_line_binfo->y[x_l], _src, sizeof(*_src) << log2_ctu_s);

    _src = &ctu_buff->cb[(max_cu_width_c - 1) * ctu_buff->stride_c];
    memcpy(&intra_line_binfo->cb[x_l>>1], _src, sizeof(*_src) << (log2_ctu_s - 1));

    _src = &ctu_buff->cr[(max_cu_width_c - 1) * ctu_buff->stride_c];
