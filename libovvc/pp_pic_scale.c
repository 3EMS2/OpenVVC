...
{
    uint16_t extra_w = (scale_info->offset_lft + scale_info->offset_rgt) << 1;
    uint16_t extra_h = (scale_info->offset_abv + scale_info->offset_blw) << 1;
    ...
    if (!luma_flag) {
        add_x = (1 - finfo->chroma_hor_col_flag) * 8 * (scale_hor - (1 << scale_bits));
        add_x = (add_x + (1 << (scale_bits - 1))) >> scale_bits ;
        add_y = (1 - finfo->chroma_ver_col_flag) * 8 * (scale_ver - (1 << scale_bits));
