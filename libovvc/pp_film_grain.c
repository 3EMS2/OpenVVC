...
int16_t
fg_compute_block_avg(const int16_t *src, uint32_t stride, uint16_t *nb_samples,
                     uint8_t blk_h, uint8_t blk_w)
{
    uint32_t sum   = 0;
    uint16_t nb_smp = 0;
    uint8_t  k, l;
    ...
    uint32_t avg = sum / nb_smp;
    avg >>= (BITDEPTH - 8);
    avg = ov_clip_uintp2(avg, 8);
...
void
fg_deblock_grain_stripe(int32_t *grainStripe, uint32_t width, uint32_t stride)
{
    int32_t lft1, lft0, rgt0, rgt1;
    uint32_t pos8, vertCtr;

    for (pos8 = 0; pos8 < (width - 8); pos8 += 8) {
        /* Across 16 pels of the vertical boundary*/
        int32_t *src = &grainStripe[pos8];
        for (vertCtr = 0; vertCtr < 16; vertCtr++) {
            ...
            src[8 - 1] = (lft1 + (lft0 << 1) + rgt0) >> 2;
            src[8 + 0] = (lft0 + (rgt0 << 1) + rgt1) >> 2;
...
void
fg_blend_stripe(int16_t *dst, const int16_t *src, int32_t *grainStripe, uint32_t blk_w,
                uint32_t blk_h)
{
    uint32_t  k, l;
    ...
    for (l = 0; l < blk_h; l++) {
        for (k = 0; k < blk_w; k++) {
            uint32_t grainSample = grainStripe[k + (l * blk_w)];
            grainSample <<=  (BITDEPTH - 8);
            dst[k + (l * blk_w)] = (int16_t) ov_clip_uintp2(grainSample + src[k + (l * blk_w)],
                                                                BITDEPTH);
        }
    }
}
...
void
fg_simulate_grain_blk8x8(int32_t *grainStripe, uint32_t grainStripeOffsetBlk8,
                         uint32_t width, uint8_t log2_scale, int16_t scaleFactor,
                         uint32_t kOffset, uint32_t lOffset,
                         uint8_t h, uint8_t v, uint32_t xSize)
{
    ...
    for (l = 0; l < 8; l++) {
        uint32_t idx_offset_l = idx_offset + (l + lOffset) * DATA_BASE_SIZE;
        uint32_t grainStripeOffsetBlk8_l = grainStripeOffsetBlk8 + (l * width);
        for (k = 0; k < xSize; k++) {
            uint32_t idx = idx_offset_l + (k + kOffset);
            grainStripe[grainStripeOffsetBlk8_l + k ] = ((scaleFactor * fg_data_base[idx]) >> (log2_scale + GRAIN_SCALE));
        }
    }
}
  ...
  uint32_t width, uint8_t log2ScaleFactor, int16_t scaleFactor, uint32_t kOffset, uint32_t lOffset, uint8_t h, uint8_t v, uint32_t xSize)
{
    ...
    for (uint32_t l = 0; l < 8; l+=4) {
      ...
      idx_offset_l1 = idx_offset + (l + 0 + lOffset) * DATA_BASE_SIZE;
      ...
      grainStripeOffsetBlk8_l1 = grainStripeOffsetBlk8 + ((l + 0) * width);
...
void
fg_data_base_generation(uint8_t enableDeblocking)
{
        ...
        for (v = 0; v < NUM_CUT_OFF_FREQ; v++) {
...
#if 0
            /*FIXME there does not seem to be a need for init this to zero */
            ...
            for (l = 0, r = 0; l <= ScaleCutOffFv; l++) {
                for (k = 0; k <= ScaleCutOffFh; k += 4) {
                    ...
                    B[k    ][l] = gaussianLUT[(pseudoRandValEhv + 0) % 2048];
    ...
    if (enableDeblocking) {
        static const uint32_t deblockFactor[NUM_CUT_OFF_FREQ] =
        {
        ...
        for (h = 0; h < NUM_CUT_OFF_FREQ; h++) {
            ...
            for (v = 0; v < NUM_CUT_OFF_FREQ; v++) {
                uint32_t dblk_factor = deblockFactor[v];
                idx_offset_v  = (idx_offset_h + v) * DATA_BASE_SIZE;
                for (l = 0; l < DATA_BASE_SIZE; l += 8) {
                    idx_offset_j  = (idx_offset_v + l) * DATA_BASE_SIZE;
                    for (k = 0; k < DATA_BASE_SIZE; k++) {
                        idx = idx_offset_j + k;
                        fg_data_base[idx] = ((fg_data_base[idx]) * dblk_factor) >> 7;
    ...
    for (int compCtr = 0; compCtr < 3; compCtr++) {
        if (fgrain->fg_comp_model_present_flag[compCtr]) {
            for (int intensityInt = 0; intensityInt < 8; intensityInt++) {
                for (int multiGrainCtr = fgrain->fg_intensity_interval_lower_bound[compCtr][intensityInt]; multiGrainCtr <= fgrain->fg_intensity_interval_upper_bound[compCtr][intensityInt]; multiGrainCtr++) {
                ...
                if (compCtr > 0) {
                    ...
                    fgrain->fg_comp_model_value[compCtr][intensityInt][1] = OVMIN(OVMAX(MIN_CUT_OFF_FREQUENCY, (fgrain->fg_comp_model_value[compCtr][intensityInt][1] << 1)), MAX_CUT_OFF_FREQUENCY );
                    fgrain->fg_comp_model_value[compCtr][intensityInt][2] = OVMIN(OVMAX(MIN_CUT_OFF_FREQUENCY, (fgrain->fg_comp_model_value[compCtr][intensityInt][2] << 1)), MAX_CUT_OFF_FREQUENCY );
                }
            }
        }
    }
    ...
    if (0 == fgrain->fg_characteristics_cancel_flag) {
        for (compCtr = 0; compCtr < 3; compCtr++) {
            ...
            for (y = 0; y < heightComp[compCtr]; y += 16) {
                    ...
                    for (blkId = 0; blkId < 4; blkId++) {
                        ...
                        if (numSamples > 0) {
                            ...
                            if (-1 != intensityInt) {
                                ...
                                fg_simulate_grain_blk8x8_sse4(grainStripe, grainStripeOffsetBlk8,
                                                              strideComp[compCtr],
                                                              log2ScaleFactor, scaleFactor,
                                                              kOffset, lOffset, h, v,
                                                              OVMIN(8, (widthComp[compCtr] - x - xOffset8x8)));
#else
                                fg_simulate_grain_blk8x8(grainStripe, grainStripeOffsetBlk8,
                                                         strideComp[compCtr],
                                                         log2ScaleFactor, scaleFactor,
                                                         kOffset, lOffset, h, v,
                                                         OVMIN(8, (widthComp[compCtr] - x - xOffset8x8)));
                ...
                fg_blend_stripe(dstSampleOffsetY, srcSampleOffsetY, grainStripe,
                                strideComp[compCtr], OVMIN(16, (heightComp[compCtr] - y)));
